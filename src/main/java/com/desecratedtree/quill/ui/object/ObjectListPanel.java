...
public final class ObjectListPanel extends JPanel {

    private int[] allIds;

    private String[] allNames;

    private final ObjectTableModel tableModel = new ObjectTableModel();
    ...
    public boolean selectObject(int id) {
    ...
    private void reloadData() {
        int count = CacheManager.getObjectCount();
        allIds = new int[count];
        allNames = new String[count];
        for (int i = 0; i < count; i++) {
            allIds[i] = i;
            ObjectDefinitions definition = ObjectDefinitions.get(i);
    ...
    private void showContextMenu(MouseEvent event) {
        ...
        menu.add(addItem);
        if (row >= 0) {
            int objectId = tableModel.getIdAt(row);
            JMenuItem duplicateItem = new JMenuItem("Duplicate");
            duplicateItem.addActionListener(e -> {
                if (duplicateAction != null) {
                    duplicateAction.accept(objectId);
                }
            });
            ...
            deleteItem.addActionListener(e -> {
                int result = JOptionPane.showConfirmDialog(
                        this,
                        "Reset object " + objectId + " to a blank default definition?",
                        "Delete Object",
                        JOptionPane.YES_NO_OPTION,
                        JOptionPane.WARNING_MESSAGE
                );
                if (result == JOptionPane.YES_OPTION && deleteAction != null) {
                    deleteAction.accept(objectId);
                }
            });
            menu.add(deleteItem);
        }
    ...
    private final class ObjectTableModel extends AbstractTableModel {
